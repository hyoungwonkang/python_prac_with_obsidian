...
`C:\dev\python-prac\`는 **파이썬 학습용 Obsidian vault**.
...
- **vault 노트(`10-Projects/_Projects.md`, 각 프로젝트 노트, `CLAUDE.md`)** = git에 올라가는 **정본**
- **메모리(`C:\Users\hwplu\.claude\projects\...\memory\`)** = 이 PC 한정 보조 캐시
- 같은 결정을 양쪽에 둘 때, **vault 노트가 우선**. 메모리가 더 오래되면 vault 기준으로 갱신.
