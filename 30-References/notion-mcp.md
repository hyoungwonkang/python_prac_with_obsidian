...
| 2 | 메인 동기 스크립트 | `sync.py` — git diff로 변경 `.md` 검출 → 변환 → API PATCH |
| 3 | 마크다운 → Notion blocks 변환 | `converter.py` (mistune 활용) |
| 4 | GitHub embed 생성 | `github_link.py` — 실습 코드 파일명 검출 → blob URL → Notion 링크 카드 |
| 5 | 의존성·문서 | `requirements.txt`, `README.md` |
| 6 | git hook 설치 | `.git/hooks/post-commit` (실행 권한 + `sync.py` 호출) |
| 7 | 이 파일의 "운영 원칙" 갱신 | "수동" → "post-commit 자동, MCP는 보조" |
| 8 | 테스트 | vault 일부 수정 → commit → Notion 자동 반영 확인 |
