...
- 🔴 **MPS 비연속(non-contiguous) 가중치 행렬곱 버그 (2026-07-02 실측, torch 2.8.0)**: `torch.tensor(numpy_전치뷰)`로 만든 Linear weight가 stride 꼬인 비연속 Parameter가 되는데, **CPU는 정상 계산·MPS는 에러 없이 틀린 값**을 냄. 증상: 사전훈련 GPT-2 loss가 3.8이 아닌 **20**에서 시작, 학습하면 횡설수설 모델이 됨. 해법: `hf_weight_adapter.assign`에 `np.ascontiguousarray` 적용(수정 완료 — CPU 3.8112 vs MPS 3.8113 일치 확인). **잔여 지뢰**: 교재 원본 `previous_7.py`(및 rasbt 공식 코드)의 `assign`에는 같은 잠재 버그가 남아 있음 — 가중치 로드는 반드시 어댑터 경유. **진단 요령**: 학습이 이상하면 같은 배치로 `calc_loss_batch`를 CPU vs MPS 두 번 계산해 비교 (값이 다르면 장치 버그, 같으면 코드/데이터 문제).
