...
### Hugging Face 모델 로드·저장 (BERT 실습에서)
- **`Model.from_pretrained("bert-base-uncased")`** — **Hugging Face Hub**에서 사전학습 모델·토크나이저를 다운로드 → `~/.cache/huggingface/`에 캐시(처음 1회만, 이후 재사용). ⚠️ bert-base-uncased는 **구글** 모델(OpenAI 아님). cf. **데이터** 다운로드는 별개(`urllib`로 UCI 등에서).
- **`BertForSequenceClassification.from_pretrained(name, num_labels=2)`** — BERT 본체 + **분류 head**(랜덤 초기화). 로드 시 "newly initialized classifier / should TRAIN" 경고는 **정상**(파인튜닝 대상). `[CLS]` 벡터→분류층(패턴 A).
- **`torch.save(model.state_dict(), "x.pt")`** / **`model.load_state_dict(torch.load("x.pt", map_location=device))`** — 학습된 **가중치(state_dict)를 `.pt` 파일로 저장/로드**. 재학습 없이 모델 복원(시연·배포용). `.pt`=파인튜닝 결과물(원본과 별개 파일).
- **device = `"mps"`/`"cuda"`/`"cpu"`** — 연산이 도는 **장소**(애플 GPU/엔비디아 GPU/CPU). 모델 부품(FFN 등)과 무관. `.to(device)`로 모델·텐서를 올림. MPS는 비결정 연산이라 시드 고정해도 결과 소수점이 약간 흔들릴 수 있음.
