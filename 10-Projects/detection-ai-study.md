...
| 4 | **PaddleOCR** | 추출 계층 (②, 이미지→텍스트) | — (라이브러리) | 라이브러리 추론(한국어 인식 정확도 확인) → 출력이 BERT/NER 입력으로 이어지는 미니 파이프라인. 커스텀 학습은 무거워 최후순위. [[../30-References/bert_ocr_practice_plan]] 연결 | 🔄 **진행 중** (2026-07-14 착수 — `rnd-ocr` 신설. EasyOCR 1.7.2 설치, CER 채점 잣대(ocr_eval.py)·열화 4종 기준선 실측: clean 9.5%·rotated 64.3%(어순 붕괴). **OpenCV 전처리 복구 실증**(2026-07-14): deskew로 rotated 64.3→9.5% 완전 복구(clean과 동일 오독 = 엔진 탓/열화 탓 분해), 단 확대는 오히려 악화 9.5→11.9%(전처리=표적 치료). **PaddleOCR 비교 완료**(2026-07-16, 격리 ocr-env·paddle 3.3.1): Paddle 5전 5승 clean 4.8%·오답 프로필 상이(Easy=모양 혼동/Paddle=끝 글자 절단)·회전은 엔진 무관 deskew 필요, **deskew+Paddle=2.4% 최고**. **미니 파이프라인 완료**(2026-07-16): OCR 3소스→스팸·NER·PII — 스팸·NER은 오독을 견디나 **PII는 Easy 한 글자(`010→0/0`)에 전멸(MEDIUM→INFO)** = ②품질이 ③성능임을 실증. 남음: 문서화(01~04)) → [[../30-References/rnd-ocr/로드맵]] |
