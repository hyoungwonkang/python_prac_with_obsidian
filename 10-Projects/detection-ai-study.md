...
| 4′ | YOLO 라벨링 직접 → 등록 학습 | 신규 — [[../30-References/rnd-dataset-artifacts/03-사용법|YOLO 데이터 규약]] 위에서 진행 | ✅ **완료** (2026-07-13 — 웹캠 촬영 16장 직접 라벨링(labelme)→변환→학습 mAP50 0.995→데모 YOLO_PT 스위칭 탐지 실증. 개인 사진은 git 제외(프라이버시). **확장 실험**: COCO+잠옷 81클래스 병합 재학습 → 잠옷 0.995 습득했으나 COCO 붕괴(0.1768) — 파국적 망각·라벨 결손 노이즈 실증, 결론=병행 운용(YOLO_PT 교체)이 실무 정답) |
