...
## MLflow 이중 기록 체계 (2026-07-07 확립)

- **학습 기록(로컬)**: `finetune_klue_tc.py`가 프로젝트 폴더의 `sqlite:///mlflow.db`에 — 에폭별 곡선 포함. UI로 보려면: `~/rnd-env/bin/python -m mlflow server --port 5001 --backend-store-uri sqlite:///<이 폴더>/mlflow.db` (5000은 통합 서버가 사용 중).
- **보고 기록(통합 서버)**: `eval_mlflow_tc.py`가 저장 가중치를 재평가해 **127.0.0.1:5000의 통합 저장소**(주간보고용 — gpt2-*·bert-spam·ner-klue와 동거)에 eval-only run 1건 기록. 실행: `TC_SUBSET=10000 ~/rnd-env/bin/python eval_mlflow_tc.py`.
- 기록 검증: 통합 서버 run `klue-bert-base-tc-7class` — 정확도 0.8425·매크로_F1 0.8313, **학습 시점 수치와 완전 일치** (.pt 왕복 무손상 재확인). `eval_mlflow_spam/ner.py` 관례의 TC판.
...
- **파라미터 연쇄** (독립이 아니라 사슬): 데이터 성격(제목=짧음) → 최대_길이 128→64 → 문장당 메모리 절반 → **배치 16→32로 증대 가능** → 1.9분 완주. Alpaca 때 "OOM이라 배치 축소"의 **역방향** 사례 — 길이를 줄인 만큼 배치를 키우는 균형점 이동.
- **배치의 역할 구분 — 평가는 무관, 학습은 유관**: 평가 2,000건은 "32개씩 63회 왕복"(62×32+16)으로 처리될 뿐, 몇 개씩 묶든 **각 건의 예측과 성적(0.8425)은 동일** — 평가에서 배치는 순전히 속도·메모리 손잡이. 반면 **학습에서는** 배치가 기울기 계산 단위라 학습 경로에 실제 영향. "몇 등분"이 아니라 "묶음 크기"이며, 나눗셈의 몫은 왕복 횟수.
